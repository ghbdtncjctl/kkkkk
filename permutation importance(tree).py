...
result = permutation_importance(
    rf, X, y, n_repeats=10, random_state= SEED, n_jobs=-1
)
...
pd.DataFrame(data={'score': abs(result.importances_mean) / sum(abs(result.importances_mean))}, 
                      index=X.columns).sort_values(by='score').plot(kind='barh', grid=True,
                                               figsize=(10,6), legend=False)
#plt.show()
...
start_time = time.time()
result = permutation_importance(
    rf_grid.best_estimator_, X, y, n_repeats=10, random_state= SEED, n_jobs=-1
)
elapsed_time = time.time() - start_time

print(f"Elapsed time to compute the importances: {elapsed_time:.3f} seconds")

sel_mean = [abs(result.importances_mean[i]) for i, support
                     in enumerate(rf_grid.best_estimator_['selector'].get_support()) if support]
    
pd.DataFrame(data={'score': sel_mean / sum(sel_mean)}, 
                      index=selected_features).sort_values(by='score').plot(kind='barh', grid=True,
                                               figsize=(10,6), legend=False)

print(pd.DataFrame(data={'score':sel_mean,
                       'support':True},
